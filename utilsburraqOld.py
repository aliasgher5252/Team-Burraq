...
def trackPerson(drone, w, pError, pError_y, pError_z, y,boxes):
  pid=[0.00175,0.0005]
  xmin=boxes[0][0]
  ymin=boxes[0][1]
  xmax=boxes[0][2]
  ...
  if ((cx!=0) and (abs(error)>30 or abs(error_y)>30)):

    speed_y=pid[0]*error_y + pid[1]*(error_y-pError_y)
    speed_y=np.clip(speed_y,-5,5)

    speed=0.004*error + 0.0005*(error-pError)
...
def wait_wp(drone,yolo,cap,out,detection=True):
  ...
  if detection:
    while(message.wp_dist>=0.5):
      ...
      _,boxes=yolo.get_bbox(frame,0.65,["X"])
      print(boxes)
      end_time=time.time()
      ...
      if(len(boxes)!=0):
        ...
        while True:
          if (len(boxes)!=0):
            ...
            out.write(frame)
            pError, pError_y, pError_z, speed_y, speed=trackPerson(drone, 320, pError, pError_y, pError_z, 320,boxes)
          else:
            out.write(frame)
